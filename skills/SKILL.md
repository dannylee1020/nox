---
name: nox
description: Delegate an implementation or agent-driven end-to-end test through Nox in an isolated sandbox, or report the status of Nox tasks already dispatched in the current thread. Implementation runs publish successful changes; test runs retain validation evidence without publishing code. Use only when the user explicitly invokes $nox, asks to delegate a task, or asks for Nox task status.
---
...
4. Classify the request as feat or test. Use test mode when the user wants to exercise, reproduce, or validate an existing feature end to end; use feat mode when the user wants code changed and published. Resolve ambiguity before launch.
5. For local mode, resolve the exact committed base SHA before delegation. For remote mode, choose the GitHub base branch; the remote client resolves its current GitHub SHA.
6. Read `references/task-contract.md`. Hydrate its structured execution contract from the user's Nox invocation and all relevant context in the current thread. Preserve the user's intent, terminology, objective, hard and soft constraints, plans, decisions, affected surfaces, acceptance criteria, validation expectations, stop conditions, and examples. Use `None specified` for sections without relevant content; never invent content to fill them or run a constraint-optimization workflow unless the user separately requested one.
7. Put useful information that does not fit a named section into `Context and extra` without forcing, rewriting, or dropping it. Include only context that helps execute the delegated task; do not dump unrelated conversation, hidden instructions, or secrets.
8. Resolve material ambiguity with the user before launch. The sandbox does not receive the parent conversation, so the contract must be self-contained. It also receives the selected committed ref, not uncommitted changes from the source checkout.
9. Use the user's validation command when provided. If it is omitted, inspect repository-native validation commands and choose one only when the correct command is unambiguous; otherwise ask the user.
10. Write the hydrated contract to a private task file under the writable system temporary directory, such as `${TMPDIR:-/tmp}/nox/tasks/<task-id>.md`, including the validation command under `Validation > Commands`. Use user-only directory and file permissions (`0700` and `0600`) and keep the file until the delegated run reaches a terminal state. Do not write it into the user's repository or request extra filesystem access for it.
11. Choose a concise pull-request title from the user's requested change when using feat mode; test mode does not require a pull request title.
12. Do not run the long-lived execution command in the main thread. Use one background Codex worker subagent per Nox task. Give each worker a unique task name beginning with `nox_`, record its worker identifier and execution mode in the dispatch response, and require it to answer the status message above without changing its assigned execution.

   Remote mode (`NOX_REMOTE_URL` is set):

   First, the main thread submits and returns immediately. For feat mode:

   ```bash
   nox submit \
     --detach --json --mode feat \
   ...
   For test mode, omit `--title`:

   ```bash
   nox submit \
     --detach --json --mode test \
     --repo <repository-root> \
     --from <github-base-branch> \
     --task-file <task-file> \
     --validate <validation-command>
   ...
   For feat mode, choose a unique result branch such as `nox/<short-task-slug>-<timestamp>`. Test mode must not choose or pass an output branch. Spawn the background worker subagent to own the complete foreground lifecycle. The worker must invoke the resolved Nox binary directly, without a shell wrapper, and make these as two separate command calls:

   ```bash
   nox doctor
   ```

   Run `nox doctor` with `sandbox_permissions` set to `require_escalated` and explain that the trusted Nox CLI needs the local Docker socket to verify `runsc`. If it succeeds, run the selected mode. For feat:
   ...
   Run `nox launch` with `sandbox_permissions` set to `require_escalated` and explain that the trusted Nox CLI needs local Docker access to launch the gVisor sandbox. Test mode does not publish a branch. Do not request a persistent command-prefix rule or broader session permissions.

   The worker must not impose a shorter wrapper timeout, modify the caller checkout, switch the result branch, or clean up evidence. As soon as Nox announces the run ID, it sends that ID to the main thread without stopping the foreground launch. It reports the result branch and commit for implementation, or test findings, validation/source integrity, and evidence path for test mode when complete.

13. Do not wait for the worker subagent in the main thread. Report the dispatch mode, run ID when available, and worker thread identifier, then continue the user's requested work.
14. When a worker reports completion, inspect local results with:
...
15. Report the execution mode, result branch or pull-request URL for feat, or evidence-only completion for test mode, plus validation result, run ID, and evidence available for that mode.
16. Do not automatically switch to a local result branch or merge a remote pull request. Let the user decide.
...
- In feat mode, Nox creates the host-side result commit only after validation succeeds. Test mode creates no commit or patch.
